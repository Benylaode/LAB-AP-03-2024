...
def tambah_film(judul):
    if judul not in films:
        films.append(judul)
        print(f"film {judul} telah dimasukkan")
    else:
        print(f"{judul} sudah ada")
...
def beli_tiket(judul_film):
    ...
    tiket = {'id': id_tiket, 'judul': judul_film, 'waktu': timestamp}
    tickets.append(tiket)
    simpan_info_tiket(tiket)
    print(f"Tiket berhasil dibeli. ID tiket anda: {id_tiket}")
    print(f"File tiket telah dibuat: {folders}/{id_tiket}.txt")
...
def simpan_info_tiket(tiket):
    with open(f"{folders}/{tiket['id']}.txt", 'w') as f:
        f.write("\n" + "-" * 45 + "\n")
        f.write("          TIKET BIOSKOP          \n")
        f.write("-" * 45 + "\n")
        f.write(f"ID Tiket   : {tiket['id']}\n")
        f.write(f"Film       : {tiket['judul']}\n")
        f.write(f"Tanggal    : {tiket['waktu']}\n")
        f.write("-" * 45 + "\n")
...
def tampilkan_tiket():
    if not tickets:
        ...
        print("Daftar tiket:")
        for index, tiket in enumerate(tickets):
            print(f"{index + 1}. {tiket['id']}")

def tampilkan_detail_tiket(ticket_id):
    for ticket in tickets:
        if ticket["id"] == ticket_id:
            print(f"Detail tiket {ticket['id']}: ")
            print("+--------------------------------------+")
            print("|           TIKET BIOSKOP              |")
            print("+--------------------------------------+")
            print(f"| ID TIKET : {ticket['id']} |")
            print(f"| FILM     : {ticket['judul']} |")
            print(f"| TANGGAL  : {ticket['waktu']} |")
            print("+--------------------------------------+")
...
def hapus_tiket(id_tiket):
    global tickets
    tickets = [tiket for tiket in tickets if tiket['id'] != id_tiket]
...
def menu_manajemen_tiket():
    while True:
        ...
        if opsi == '1':
            ...
            if nomor_tiket.isdigit() and 0 < int(nomor_tiket) <= len(tickets):
                tampilkan_detail_tiket(tickets[int(nomor_tiket) - 1]['id'])
        elif opsi == '3':
            tampilkan_tiket()
            nomor_tiket = input("Pilih nomor tiket yang ingin dihapus: ")
            if nomor_tiket.isdigit() and 0 < int(nomor_tiket) <= len(tickets):
                hapus_tiket(tickets[int(nomor_tiket) - 1]['id'])
        elif opsi == '4':
            break
...
def menu_pengunjung():
    while True:
        ...
        if opsi == '1':
            ...
            if pilihan_film.isdigit() and 0 < int(pilihan_film) <= len(films):
                beli_tiket(films[int(pilihan_film) - 1])
        elif opsi == '3':
            break
